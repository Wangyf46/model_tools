...
class Runner(object):
    ...
    def init_optimizer(self, optimizer):
        ...
        if isinstance(optimizer, dict):
            # optional we want to set different lr / weight_decay to weights and bias
            # which is done in maskrcnn_benchmark
            base_lr = optimizer['lr']
            base_weight_decay = optimizer['weight_decay']
            bias_lr_factor = optimizer.get('bias_lr_factor', 1)
            bias_weight_decay_factor = optimizer.get('bias_weight_decay_facotr', 1)
            ...
            params = [
                ...
                {'params': bias_params, 'lr': base_lr * bias_lr_factor, 'weight_decay': base_weight_decay * bias_weight_decay_factor}
            ]

            if 'bias_lr_factor' in  optimizer.keys():
                optimizer.pop('bias_lr_factor')

            if 'bias_weight_decay_facotr' in optimizer.keys():
                optimizer.pop('bias_weight_decay_facotr')
