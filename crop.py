...
def crop(args):
    ...
    idx = 0
    files_list = os.listdir(args.imgpath)
    for fname in files_list:
        name = os.path.splitext(fname)
        if name[1] == '.png' or name[1] == '.jpeg' or name[1] == '.png':
            src_img = cv2.imread(os.path.join(args.imgpath, fname))
            src_x0, src_y0 = src_img.shape[0] // 2, src_img.shape[1] // 2   ## TODO: center point
            x1, y1 = src_x0 - (args.height)//2, src_y0 - (args.width)//2
            x2, y2 = src_x0 + (args.height)//2, src_y0 + (args.width)//2
            ...
            ## parse and save xml
            ori_parse = parse_xml(os.path.join(args.xmlpath, name[0]) + '.xml')
            ...
            for obj in ori_ann:
                ...
                if xmin > xmax or ymin > ymax:
                    new_obj_ann['bndbox'] = [0.0, 0.0, 0.0, 0.0]
                else:
                    ## bndbox area
            ...
            shutil.copy(os.path.join(args.xmlpath, name[0]) + '.xml', dst_xml)
...
def parse_args():
    ...
    parser.add_argument('imgpath', help='Path incluing images')
    parser.add_argument('xmlpath', help='Path incluing xmls')
    parser.add_argument('dstpath', help='Path to save croped images and corresponding xmls')
    parser.add_argument('--height', type=int, default=360, help='Croped area height')
    parser.add_argument('--width', type=int, default=640, help='Croped area width')
    parser.add_argument('--threshold', type=int, default=0.3, help='Iou threshold')
